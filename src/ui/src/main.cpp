...
namespace ic::ui {
std::vector<Window*> WINDOW_LIST = INIT_WINDOW_LIST();

/** Attempts to reduce resource usage if no animation is being played. */
static bool _save_power(void);

// GL GLSL Versions
static const char* _get_glsl();
...
int run()
{
    ...
    const char* GLSL_VERSION = _get_glsl();
    ...
    std::string window_title = APPNAME;
    while (!glfwWindowShouldClose(window)) {
        ...
        if (scene != nullptr) {
            ...
            if (window_title != scene->name().data()) {
                window_title     = scene->name().data();
                std::string name = window_title + " - " APPNAME;
                glfwSetWindowTitle(window, name.c_str());
            }
        } else {
            if (window_title != APPNAME) {
    ...
    ImGui::SaveIniSettingsToDisk(_inistr.c_str());
...
static bool _save_power(void)
{
    ...
    constexpr float FPS_IDLE   = 12.f; // FPS while saving
    constexpr double TOLERANCE = 0.9;
    double before_wait         = _timediff();
    double wait_timeout        = 1. / (double)FPS_IDLE;

    glfwWaitEventsTimeout(wait_timeout);

    double after_wait    = _timediff();
